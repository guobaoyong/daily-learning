        ...
        columnDefs: [{ // 定义表格样式
            ...
            render: function (data, type, row, meta) {
                return '<td><img onClick="house_edit(\'查看\', \'/house/show?id=' + row.first.id + '\')" title="查看"' +
                    ' class="picture-thumb" src="http://images.qqzsh.top/' + data + '?imageView2/1/w/200/h/100"></td>';
            }
        }, {
            targets: 7,
            render: function (data, type, row, meta) {
            ...
            targets: 8,
            render: function (data, type, row, meta) {
                return '<a style="text-decoration: underline" onclick="userTip(' + row.second.id + ')">查看用户信息</a>'
            }
        }, {
            targets: 9,
            render: function (data, type, row, meta) {
...
function userTip(userId) {
    $.get('/admin/user/y' + userId, function (res) {
        layer.open({
            type: 1,
            title: false, //不显示标题栏
            closeBtn: false,
