...
var table = $('#data-table').DataTable({
    ...
    columnDefs: [{
        ...
        render: function (data, type, row, meta) {
            var html = '';
            if (data === 'city') {
                html = '<td class="td-status"><span class="label label-danger radius">城市</span></td>';
            } else if (data === 'region') {
                html = '<td class="td-status"><span class="label label-success radius">区域</span></td>';
            }
            return html;
        }
    },{
        ...
        render: function (data, type, row, meta) {
            var prefix = '<td class="f-14 td-manage">',
                ...
                suffix = '<a style="text-decoration:none" class="ml-5"' +
                    ' onClick="city_del(this, ' + row.id + ')" href="javascript:;" title="删除"><i' +
