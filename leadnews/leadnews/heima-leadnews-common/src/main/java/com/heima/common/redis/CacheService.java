...
@Slf4j
public class CacheService implements CachingConfigurer {
     ...
     * 使用管道技术刷新任务队列
     *
     * @param futureKey  添加了前缀的缓存主键
     * @param currentKey 添加了前缀的缓存主键
     * @param values     需要刷新的值
     */
    public List<Object> refreshWithPipeline(String futureKey, String currentKey, Collection<String> values) {
        return stringRedisTemplate.executePipelined(new RedisCallback<>() {
            @Nullable
            @Override
            public Object doInRedis(@NotNull RedisConnection redisConnection) throws DataAccessException {
                StringRedisConnection stringRedisConnection = (StringRedisConnection) redisConnection;
                String[] strings = values.toArray(new String[0]);
                stringRedisConnection.rPush(currentKey, strings);
    ...
    public String tryLock(String name, long expire) {
        ...
        try {
            ...
            Boolean result = null;
            if (conn != null) {
                result = conn.stringCommands().set(
                        name.getBytes(),
                        token.getBytes(),
