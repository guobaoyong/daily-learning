...
public class ImageSimilarity {
    ...
    public void repeat() throws IOException {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    for (int j = 0; j < files.length; j++) {
                        String result = String.valueOf(ImageSimilarity.calSimilarity(files[index], files[j]));
                        System.out.println(Thread.currentThread().getName()+",正在进行：i="+index+",j="+j+",结果："+result+","+result.equals("1.0"));
                        if (result.equals("1.0")){
                            System.out.println("出现重复，i="+index+",j="+j);
                            if (index != j){
                                System.out.println("存储成功，i="+index+",j="+j);
                                delete.put(files[index].getName(),files[j].getName());
                            }
                        }
                    }
                }
        ...
        /*for (int i = 0; i <files.length ; i++) {
            for (int j = 0; j < files.length; j++) {
                String result = String.valueOf(ImageSimilarity.calSimilarity(files[i], files[j]));
                log.info(Thread.currentThread().getName()+",正在进行：i="+i+",j="+j+",结果："+result+","+result.equals("1.0"));
                if (result.equals("1.0")){
                    log.info("出现重复，i="+i+",j="+j);
                    if (i != j){
                        log.info("存储成功，i="+i+",j="+j);
                        delete.put(files[i].getName(),files[j].getName());
                    }
                }
            }
        }*/
        ...
        for (int i = 0; i <files.length ; i++) {
            String result = String.valueOf(ImageSimilarity.calSimilarity(files[0], files[i]));
            log.info(Thread.currentThread().getName()+",正在进行：i="+i+",结果："+result+","+result.equals("1.0"));
            if (result.equals("1.0")){
                log.info("出现重复，i="+i);
                if (i != 0){
                    log.info("存储成功，i="+i);
                    delete.put(files[i].getName(),files[0].getName());
                }
            }
        }
        ...
        // 遍历集合，进行文件删除
        delete.forEach((k, v) -> {
            File file1 = new File(downloadPath + k);
            try {
                file1.delete();
            }catch (Exception e){ }
        });
